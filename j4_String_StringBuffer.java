...
class j4_String_StringBuffer
 {
	...
	StringBuffer s = new StringBuffer("Computer");
		int p = s.length();
		int q = s.capacity();
		System.out.println("Length of string Computer =  " + p);
		System.out.println("Capacity of string Computer=  " + q);
    s.append(" Applications");
    System.out.println("After Appending=  "+s);
    s.insert(11, "-");
    System.out.println("After inserting - @position 11=  "+s);
    s.reverse();
    System.out.println("After reversing =  "+s);
    s.delete(0, 2);
            System.out.println("After deleting the array from 0 to 2 position=  "+s);
            s.deleteCharAt(1);
